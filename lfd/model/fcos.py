...
class FCOS(nn.Module):
    ...
    def generate_point_coordinates(self, feature_map_sizes):
        ...
        def generate_for_single_feature_map(func_feature_map_size, func_stride):
            ...
            # CAUTION: in the original implementation: + stride // 2, but in theory, stride // 2 is unnecessary
    ...
    def annotation_to_target(self, all_point_coordinates_list, gt_bboxes_list, gt_labels_list):
        """
        :param all_point_coordinates_list: generated by generate_point_coordinates
        :param gt_bboxes_list: list: torch.Tensor([[x,y,w,h],...])
        :param gt_labels_list: lsit: torch.Tensor([label1, label2,...])
        :return:
        """
        ...
        def generate_target_for_single_image(func_gt_bboxes, func_gt_labels, func_concat_point_coordinates, func_concat_regress_ranges):
            """
            foreground lables ∈[0,num_classes-1], background label is num_classes
            :param func_gt_bboxes: [[x,y,w,h],...]
            :param func_gt_labels: [label1, labels2]
            :param func_concat_point_coordinates:
            :param func_concat_regress_ranges:
            :return:
            """
            assert func_gt_bboxes.size(0) == func_gt_labels.size(0)
            num_points = func_concat_point_coordinates.size(0)
            num_gt_bboxes = func_gt_labels.size(0)
            if num_gt_bboxes == 0:  # when no gt_bboxes, return
                return func_gt_labels.new_full((num_points,), self._num_classes), \
                       func_gt_bboxes.new_zeros((num_points, 4))

            # compute gt_bboxes' areas, and expand
            ...
            # get x, y coordinates and expand
            ...
            # compute distance to left, right, top, bottom
            ...
            # indicate whether the points fall into the gt bboxes
            inside_gt_bbox_indicator = regress_targets.min(dim=-1)[0] > 0

            # indicate which gt bboxes to learn
...
class FCOSv1(nn.Module):
    """
    这个版本主要用于实验单个point预测多个类别
    """
    def __init__(self,
                 backbone=None,
                 neck=None,
    ...
    def annotation_to_target(self, all_point_coordinates_list, gt_bboxes_list, gt_labels_list):
        ...
        def generate_target_for_single_image(func_gt_bboxes, func_gt_labels, func_concat_point_coordinates, func_concat_regress_ranges):
            ...
            # 替换为以下表达式
            # classification_targets = classification_targets * (min_areas != INF) + self._num_classes * (min_areas == INF)
            # --- 以上是原本的FCOS中获取标签 target 的做法， 针对单个point多个标签的情况，我们需要初始化一个PxC的target张量用于保存新的多类target结果
            classification_targets = func_gt_labels.new_ones((num_points, self._num_classes))
            row_indexes, column_indexes = torch.where(valid_indicator)
            class_indexes = func_gt_labels[column_indexes]
            classification_targets[row_indexes, class_indexes] = 0  # 每个point的每个类别位置都视为二分类，前景用0表示。此时classification_targets的shape为PxC
    ...
    def get_loss(self, predict_outputs, annotation_batch, *args):
        ...
        # 对预测的三类feature map进行调整，使其能够和target对齐
        batch_size = predict_classification_tensor.shape[0]
        flatten_predict_classification_tensor = predict_classification_tensor.reshape(-1, 1)  # 这里直接将整个tensor拉平, 但是需要保留一个维度
        flatten_predict_regress_tensor = predict_regress_tensor.reshape(-1, 4)
        flatten_predict_centerness_tensor = predict_centerness_tensor.reshape(-1)

        # 这里需要在把 classification tensor 拉平之前 获取pos indexes
        classification_target_tensor = classification_target_tensor.reshape(-1, self._num_classes)
        pos_indexes = (classification_target_tensor == 0).sum(dim=1).nonzero().reshape(-1)
